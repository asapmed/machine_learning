...
class KNearestNeighbor:
    ...
    def compute_distances_one_loop(self, X):
        ...
        sum2 = (self.X_train ** 2).sum(axis=1)
        for i in range(num_test):
            #######################################################################
            # TODO:                                                               #
            ...
            dists[i] = (sum2 + (X[i] ** 2).sum() - 2 * np.dot(self.X_train, X[i]))**0.5
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = ((self.X_train ** 2).sum(axis=1) + (X[:,:,None] ** 2).sum(axis=1) - 2 * np.dot(X, self.X_train.T))**0.5
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            nearest_indices = np.argpartition(dists[i], k)[:k]
